...
import ArticleSummary from '@/components/organisms/ArticleSummary/ArticleSummary.astro';
...
async function handleTypeCreation() {
  ...
  createTypes(response.types.reduce((acc, type) => acc.concat(createTypeDef(schema, type)), []));
}
```
...
async function handleNodeCreation() {
  ...
  const allItems = unionBy(response.items, response.linkedItems, 'system.codename');
